...
async function main() {
  ...
  try {
    ...
    if (!migrationTableSchema) {
      console.warn(
        "migration status: drizzle migration テーブルが見つかりません。",
      );
      console.warn(
        "=> db:push 実行済み、またはまだ `drizzle migrate` が未実行の可能性があります。",
      );
      console.warn(
        "=> migration テーブルがないため、migration 履歴比較は実行できません。",
      );
    ...
    if (extraMigrations.length > 0) {
      ...
      for (const migration of extraMigrations) {
        const migrationFile =
          migration.hash === null ? undefined : localByHash.get(migration.hash);
    ...
    if (!latest) {
      ...
      const latestFile =
        latest.hash === null ? undefined : localByHash.get(latest.hash);
